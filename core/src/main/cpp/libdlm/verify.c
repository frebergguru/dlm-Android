...
{
    /* expected_hex is NULL when the source (e.g. archive.org) omits a checksum;
     * treat "nothing to compare against" as an error, not a crash. */
    if (!expected_hex) return DLM_VERIFY_ERROR;
    char got[33];
